...
const mealTypeLabel = computed(() => {
  ...
  return labels[meal.value?.mealType] || 'Другое'
})
...
async function loadMeal() {
  ...
  try {
    meal.value = await mealService.getMealById(route.params.id)
    grams.value = meal.value.amountGrams

    food.value = await foodService.getProductById(meal.value.foodId)
  } catch (e) {
    error.value = e.response?.data?.error || 'Не удалось загрузить запись'
  } finally {
    loading.value = false
  }
}
...
async function saveMeal() {
  ...
  try {
    await mealService.updateMeal(route.params.id, {
      weight: Number(grams.value),
    })

    success.value = 'Запись успешно обновлена'

    setTimeout(() => {
      router.push('/diary')
    }, 600)
  } catch (e) {
    error.value = e.response?.data?.error || 'Не удалось обновить запись'
  } finally {
    loading.value = false
  }
}
...
async function deleteMeal() {
  const confirmed = confirm('Удалить продукт из дневника?')
  ...
  try {
    await mealService.deleteMeal(route.params.id)
    router.push('/diary')
  } catch (e) {
    error.value = e.response?.data?.error || 'Не удалось удалить запись'
  } finally {
    loading.value = false
  }
}
...
<template>
  <div class="mx-auto max-w-2xl space-y-6">
    ...
    <div class="rounded-3xl bg-white p-6 shadow-sm">
      <div v-if="loading && !meal" class="text-gray-500">Загружаем запись...</div>
      ...
      <template v-if="meal && food">
        ...
        <div class="mt-6 flex flex-col gap-3 md:flex-row">
          ...
          <button
            type="button"
            class="flex-1 rounded-xl border border-red-200 py-3 font-semibold text-red-600 hover:bg-red-50 disabled:opacity-60"
            :disabled="loading"
            @click="deleteMeal"
          >
            Удалить
